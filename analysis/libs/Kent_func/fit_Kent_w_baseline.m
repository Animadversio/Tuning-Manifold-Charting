function [Parameter, gof] = fit_Kent_w_baseline(funcval)
ft = fittype( @(theta, phi, psi, kappa, beta, A, bsl, x, y) bsl + KentFunc(theta, phi, psi, kappa, beta, A, x, y), ...
...
[Parameter, gof] = fit( [theta_grid(:), phi_grid(:)], [funcval(:)], ft, ...
                'StartPoint', [0, 0, pi/2, 0.1, 0.1, 0.1, 0.01], ...
                'Lower', [-pi, -pi/2,   0, -Inf,   0,   0,   0], ... % another version is to enforce kappa non negative 
                'Upper', [ pi,  pi/2,  pi,  Inf, Inf, Inf, Inf]);%, ...
gof.coefname = string(coeffnames(Parameter)');
gof.coef = coeffvalues(Parameter);
gof.confint = confint(Parameter);
% "theta"    "phi"    "psi"    "kappa"    "beta"    "A"  "bsl" sequence
return